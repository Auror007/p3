...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["registernew-registernew-module"], {
  ...
  function srcAppRegisternewRegisternewPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var RegisternewPage = /*#__PURE__*/function () {
      function RegisternewPage(regServ, http, router, nav, storage) {
        var _this = this;
        ...
        this.storage.get('addr').then(function (data) {
          ...
          _this.regServ.setAddress(_this.address);
        });
        this.storage.get('cord').then(function (data) {
          ...
          _this.regServ.setLat(_this.lat);

          _this.regServ.setLng(_this.lng);
        });
      }
          ...
          try {
            this.storage.get('activity').then(function (result) {
              if (result == 'registered') {
                ...
                _this2.http.post('https://mywash.herokuapp.com/verifyregister', data).subscribe(function (result) {
                  console.log(result);

                  if (result.message == true) {
                    _this2.storage.set('num', data.phone);

                    _this2.router.navigate(['/addvehicle'], {
                      replaceUrl: true
                    });
                  }
                }, function (error) {
                ...
                _this2.http.post('https://mywash.herokuapp.com/verifylogin', data1).subscribe(function (result) {
                  console.log(result);

                  if (result.message == true) {
                    _this2.storage.set('activity', 'loggedin');

                    _this2.storage.set('address', result.message);

                    _this2.storage.set('coords', {
                      lat: result.lat,
                      lng: result.lng
                    });

                    _this2.router.navigate(['/tabs/tabs/dash'], {
                      replaceUrl: true
                    });
                  }
                }, function (error) {
